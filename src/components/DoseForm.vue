    ...
    <mdui-card variant="" class="form-card" style="width: 100%;">
      <div class="form-content">
        <div class="selected-drug-banner" v-if="drug">
          ...
          <div class="banner-info">
            ...
            <span class="banner-detail">
              ...
              <template v-if="drug.parameters?.equivalence_factor">· 等效系数={{ drug.parameters.equivalence_factor }}</template>
              · t½={{ drug.parameters?.half_life || '?' }}h
            </span>
          </div>
        </div>
        ...
        <mdui-text-field
          ...
          type="number" variant="outlined" @input="e => $emit('update:doseAmount', e.target.value)"
        ></mdui-text-field>

        <mdui-select :value="route" label="给药方式" variant="outlined" @change="e => $emit('update:route', e.target.value)">
        ...
        <mdui-text-field
          ...
          @input="e => $emit('update:timestamp', e.target.value)"
        ></mdui-text-field>
